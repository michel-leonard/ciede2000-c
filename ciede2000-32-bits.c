...
static float ciede2000_with_parameters(float l1, float a1, float b1, float l2, float a2, float b2, float kl, float kc, float kh, int canonical) {
	...
	const float hue_wrap = (float) (pi_1 + 0.000001f < delta_h);

	// The part where most programmers get it wrong
	const float h_mean = h_mean_raw - (canonical && pi_1 + 0.000001f < h_mean_raw ?
		1.0f : // canonical=1 acts like Gaurav Sharma, OpenJDK, ...
		-1.0f  // canonical=0 acts like Bruce Lindbloom, Netflix’s VMAF, ...
	) * hue_wrap * pi_1;
...
static inline float ciede2000(float l1, float a1, float b1, float l2, float a2, float b2) {
	return ciede2000_with_parameters(l1, a1, b1, l2, a2, b2, 1.0f, 1.0f, 1.0f, 0);
}
